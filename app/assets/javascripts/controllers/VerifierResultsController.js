angular.module('verifier')
	.controller("VerifierResultsController", [ "$scope", "$http", "$location", "crudService",'$window', "$routeParams",
...
$scope.searchName = function(searchTerm, dietId){ 
    // if (searchTerm.length < 10){
    //   return;
    // }
    $scope.results = [];
...
//debugger
    ...
    if (c === null) {
		...
		$http.get("/exams.json", 
 		 ...
 		 $http.get("/waec_exams/SearchCandidateFromVerifier.json", 
		 		   {"params": { "CandNo": searchTerm, "dietId": diet_id}}
      		).then(function(response){ 
...
// debugger
	...
	$scope.viewDetails = function(result){
		...
		//console.log(examYear, result)
			...
			$location.path("result/verifier_search/" + result.examYear +"/"+ result.candNo  );
		}


	$scope.addressDetails = function(result){

			$location.path("result/verifier_search/address/"+ result.examYear +"/"+ result.candNo );
...
//debugger
	 ...
	 //let year_id = ExamDietService.getYear();
		...
		$http.get("/waec_exams/SearchCandidateFromVerifier.json", 
				   ...
				   {"params": { "CandNo": $routeParams.examNo, "dietId": diet_id}}
			).then(function(response){ 
    ...
    $scope.loadConfirmImage = function (record) {
					  // body...
				  
		record.confirmImage.candNo = $routeParams.examNo;
		record.confirmImage.dietId = window.sessionStorage.getItem('examID');

        console.log(record.confirmImage);

		$http({
				method: 'POST',
				url: " /confirmation_images.json",
				data: record.confirmImage,
		
				header: {
					'Content_Type' : 'application/json'
					}
		}).then(function(response){
				alert('Candidate Image Uploaded successfully.');
  ...
  $scope.confirms = [];
		...
		$scope.getAllConfirmation = function(){
			 $scope.loading = true;
		    		$http({
		    		method: 'GET',
		    		url: '/waec_exams/confirmations.json',
					}).then(function(response){
					    		$scope.confirms = response.data;
					    		//console.log($scope.confirms);
