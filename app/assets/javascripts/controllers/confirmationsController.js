angular.module('verifier')
	.controller("ConfirmationsController", [ "$scope", "$http", "$location", "crudService",'$window', "$routeParams",
		...
		function ( $scope, $http, $location, crudService,$window, $routeParams, ExamDietService, ConfirmationService, 
			fileUpload, ResultService, $route, WebClient, userSession,SweetAlert, toaster) {
var vm = this ;
	 ...
	 //let year_id = ExamDietService.getYear();
		...
		$http.get("/exams.json", 
				   {"params": { "CandNo": searchTerm, "yearId": year_id, "dietId": diet_id}}
							    ...
							    $scope.resultAddress = response.data[0];
...
$scope.BlobPdfFile = {};
    	...
    	$scope.selectConfirm  = {};
    		...
    		//$scope.selectConfirm = confirm; 
    		$location.path('/confirmations/Details/'+confirm.id);  
    		$scope.selectConfirm = confirm; 			
...
debugger
				...
				if ($scope.resultDetail[0].DOB == null) {
					$scope.resultDetail[0].DOB = 'N/A';
						...
						//`${subHeading}\n\n`
												  text:[
																		  {text: $scope.resultDetail[0].Diet+' '+ $scope.resultDetail[0].Year+'\n'},
												...
												//widths: ['*', '*', '*'],
												  ...
												  table: {
														  ...
														  body: [
																  ...
																  [{text:$scope.resultDetail[0].Subject}, {text:$scope.resultDetail[0].Grade}, {text:$scope.gradeIntepretation( $scope.resultDetail[0].Grade)}],
																  [{text:$scope.resultDetail[1].Subject}, {text:$scope.resultDetail[1].Grade}, {text:$scope.gradeIntepretation( $scope.resultDetail[1].Grade)}],
																  [{text:$scope.resultDetail[2].Subject}, {text:$scope.resultDetail[2].Grade}, {text:$scope.gradeIntepretation( $scope.resultDetail[2].Grade)}],
																  [{text:$scope.resultDetail[3].Subject}, {text:$scope.resultDetail[3].Grade}, {text:$scope.gradeIntepretation( $scope.resultDetail[3].Grade)}],
																  [{text:$scope.resultDetail[4].Subject}, {text:$scope.resultDetail[4].Grade}, {text:$scope.gradeIntepretation( $scope.resultDetail[4].Grade)}],
																  [{text:$scope.resultDetail[5].Subject}, {text:$scope.resultDetail[5].Grade}, {text:$scope.gradeIntepretation( $scope.resultDetail[5].Grade)}],
																  [{text:$scope.resultDetail[6].Subject}, {text:$scope.resultDetail[6].Grade}, {text:$scope.gradeIntepretation( $scope.resultDetail[6].Grade)}],
																  [{text:$scope.resultDetail[7].Subject}, {text:$scope.resultDetail[7].Grade}, {text:$scope.gradeIntepretation( $scope.resultDetail[7].Grade)}],
																  [{text:$scope.resultDetail[8].Subject}, {text:$scope.resultDetail[8].Grade}, {text:$scope.gradeIntepretation( $scope.resultDetail[8].Grade)}],
																  [{text:'NO OF SUBJECTS LISTED:', margin:[0,10,0,0]}, {text: $scope.subjOrdinal($scope.resultDetail[0].NoOfSubjects)+' (' + $scope.resultDetail[0].NoOfSubjects+')', margin:[0,10,0,0]}, {text:''}],
														  ]
												  },
												  layout: 'noBorders'
	...
	ResultService.ResultDetailsFromDB(newValue.exam_no, newValue.YearName, newValue.DietName)
			...
			// body...
					...
					$http.get("/users/permitted_users",{"params": { "email": $scope.userID}} ).then(function(response){
						...
						//If Candidate Result doesn't contain DOB
						if ($scope.resultDetail[0].DOB == null) {
							$scope.resultDetail[0].DOB = 'N/A';
						}
