angular.module('verifier')
	.controller("ConfirmPaymentController", [ "$scope", "$http", "$location", "crudService",'$window', "$routeParams",
		'ExamDietService',"ConfirmationService", "ResultService", 'ReceiptListService', 'Receipt', 'myAppFactory', '$filter','uibDateParser','CSVConverterService',
		function ($scope, $http, $location, crudService,$window, $routeParams, ExamDietService, 
			ConfirmationService, ResultService, ReceiptListService, Receipt, myAppFactory, $filter, uibDateParser, CSVConverterService) {
...
$scope.search = function(searchTerm, ExamYear){ 
...
//debugger
    ...
    if (c === null) {
          ...
          $http.get("/exams/getResultByCandidate.json", 
           {"params": { "candName": searchTerm, "dietId": diet_id, "year": $scope.year}}
      ...
      , function(response){ 
        alert("There was a problem: " + response.status); 
         $scope.loading = false;
          ...
          $http.get("/exams/getResultStatus.json", 
           {"params": { "CandidateNo": searchTerm, "ExamYear": ExamYear}}
      ).then(function(response){ 
    ...
    // $http.get("/exams/getResultStatus.json", 
    //        {"params": { "CandNo": searchTerm, "dietId": diet_id, "yearId": year_id}}
    //   ).then(function(response){ 
  ...
  $scope.viewDetails = function(result){
    console.log(result);
    if(result.CandNo != null){
        var exam_no = result.CandNo;
    }
    else{
        var candNo= result.candNo;
        var examYear= result.examYear;
    }



      $location.path("/verifer/search/"+ examYear+"/"+ candNo );
    }
...
//loadResultDetail(result)
    $scope.result = [];
  $scope.loadResultDetail = function (exam_no) {
    $scope.loading = true;
   ...
   // Return All Confirmations to  View...............
      ...
      $scope.loading = true;
        ...
        $scope.getTotal = function(){
          var total = 0;
          for(var i = 0; i < $scope.confirms.length; i++){
              total = total + parseInt($scope.confirms[i].amount);
          }
          return total;
      }
...
$scope.today = function() {
  ...
  $scope.dateFrom = $filter('date')(dateFrom, 'yyyy-MM-dd'); //dateFrom;
    ...
    $scope.dateTo = $filter('date')(dateTo, 'yyyy-MM-dd');// dateTo;
	  ...
	  $scope.exportJSONToCsv = function(confirms){
	  	// body...confirm.office_name
	  	...
	  	var exportData = [];
            confirms.forEach(function (confirm) {
                exportData.push({
                    ...
                    'Transaction': confirm.transName,
                    'Transactn Type': confirm.typeName,
                    'Amount': confirm.amount,
                    'Receipt No': confirm.receipt_no,
                    'Staff Name': confirm.fullname,    
                    'Office': confirm.office_name,
                    'Trans-Date': $filter('date')(confirm.created_at, 'shortDate')
                });
            });

	  	CSVConverterService.downloadFile(exportData, 'jsontocsv');
	  	
	  }



      $scope.exportToCsv = function (confirms) {
            var exportData = [];
            confirms.forEach(function (confirm) {
                exportData.push({
                    'Candidate Name': confirm.CandName,
...
debugger
    var headers = {
    fila_0:{
        col_1:{ text: 'Candidate Name', style: 'tableHeader',alignment: 'left',margin: [0, 8, 0, 0] },
        col_2:{ text: 'Contact', style: 'tableHeader',alignment: 'left',margin: [0, 8, 0, 0] },
        col_3:{ text: 'Transaction', style: 'tableHeader',alignment: 'left',margin: [0, 8, 0, 0] },
        col_4:{ text: 'Transactn Type', style: 'tableHeader',alignment: 'left',margin: [0, 8, 0, 0] },
        col_5:{ text: 'Amount', style: 'tableHeader',alignment: 'left',margin: [0, 8, 0, 0] },
        col_6:{ text: 'Receipt No', style: 'tableHeader',alignment: 'left',margin: [0, 8, 0, 0] },
        col_7:{ text: 'Staff Name', style: 'tableHeader',alignment: 'left',margin: [0, 8, 0, 0] },
        col_8:{ text: 'Office', style: 'tableHeader',alignment: 'left',margin: [0, 8, 0, 0] },
        col_9:{ text: 'Trans-Date', style: 'tableHeader',alignment: 'left',margin: [0, 8, 0, 0] }
      }
    }
  ...
  for (var key in rows){
    ...
    if (rows.hasOwnProperty(key))
    {
      ...
      row.push(data.CandName.toString());
      row.push(data.PhoneNo.toString());
      row.push(data.transName.toString());
      row.push(data.typeName.toString());
      row.push(data.amount.toString());
      row.push(data.receipt_no.toString());
      row.push(data.fullname.toString());
      row.push(data.office_name.toString());  
      row.push($filter('date')(data.created_at, 'dd-MM-yyyy'));
      body.push(row);
    }
        ...
        var dd = {
          ...
          content: [
                  ...
                  },
                  {
                    ...
                    columns:[
                        ...
                        [
                          {text:'Office: '+ $scope.confirms[1].office_name.toString()},
          ...
          {
            text: 'Total Amount: '+ $scope.getTotal().toString(),
          },
          ],
