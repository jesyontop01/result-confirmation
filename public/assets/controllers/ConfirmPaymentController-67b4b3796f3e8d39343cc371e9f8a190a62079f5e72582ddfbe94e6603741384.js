...
//debugger
    ...
    if (c === null) {
          ...
          $http.get("/exams/getResultByCandidate.json", 
           {"params": { "candName": searchTerm, "dietId": diet_id, "yearId": year_id}}
  ...
  $scope.viewDetails = function(result){
    ...
    if(result.CandNo != null){
      ...
      $location.path("/verifer/search/" + exam_no );
    }
...
//loadResultDetail(result)
  ...
  $scope.loadResultDetail = function (exam_no) {
    $scope.loading = true;
        ...
        console.log($routeParams.exam_no);
        let searchTerm = $routeParams.exam_no;
  ...
  $scope.getDate = function(WaecOfficeId, dateFrom, dateTo) {
	...
	$scope.dateFrom = $filter('date')(dateFrom, 'yyyy-MM-dd'); //dateFrom;
      ...
      $scope.exportToCsv = function (confirms) {
            var exportData = [];
            // exportData.forEach(function (confirm) {
            //     exportData.push({
            //         'Candidate Name': confirm.CandName,
            //         'Amount': confirm.amount,
            //         'Receipt No': confirm.receipt_no,
...
var dd = {
    content: [
        ...
        table(confirms, ['id', 'CandName', 'PhoneNo', 'receipt_no', 'amount','transName','office_name', 'typeName', 'cand_email', 'created_at',])
    ]
}
