angular.module('verifier')
	.controller("ConfirmationAddressController", ["$scope", "$http", "$routeParams",'$window','ConfirmationService','$location','ResultService',
										function($scope, $http, $routeParams, $window , ConfirmationService, $location, ResultService){

			$scope.examDiet = $routeParams.examDiet;
			console.log($scope.examDiet);
			$scope.examYear = $routeParams.examYear;
			console.log($scope.examYear);
			 //$scope.ExamAndYear = sessionStorage.getItem('ExamY');


			$scope.exam_no = $routeParams.exam_no;
			console.log($routeParams);

			
    	// Functions to Open And Close Modal Pop-Up...........
    	...
    	$scope.selectConfirm  = {};
    		...
    		if(confirm.dietTbl_id != null){
				...
				console.log($scope.examDiet);
				console.log($scope.examDiet.Diet);
				$scope.examDietName = $scope.examDiet.diet;
			},function(response){
    		...
    		if(confirm.year_id != null){
				...
				console.log($scope.examYear.Year);
				$scope.examYearName = $scope.examYear.Year;
			},function(response){
				alert(response.status);
			});
    		}
    			...
    			$http({
					...
					header: {
						'Content_Type' : 'application/json'
							    ...
							    $scope.success = true;
							    $scope.error = false;
							    $scope.successMessage = 'confirmation was updated successfully.';
...
console.log( $scope.confirmType);
     ...
     var countryId = $scope.applicant.confirm_type_id;
     if (countryId) {
     			$http.get("/confirm_countries.json", 
...
$scope.confirm_countries_id = {};


    	$scope.saveNewConfirmation = function(){

	    	// Setting Modal parameters.........................	
			  $scope.modalTitle = 'Add Confirmation Address';
			  $scope.submit_button = 'Create Confirmation';
			 // $scope.openModal();
	     ...
	     $scope.student = {
		    ...
		    diet_id: idDiet,
		    year_id: idYear,
		    exam_no: $routeParams.exam_no,
		    receipt_no: null,
		    Cand_address: null,
		    confirm_type_id: null,
		    dest_title: null,
		    dest_address1: null,
		    dest_address2: null,
		    dest_location: null,
		    confirm_country_id: null,
		    dest_email: null

		 };

		  $scope.applicant = angular.copy($scope.student);

			
					
				$scope.createConfirm = function(applicant){

							console.log($scope.applicant);

							angular.copy(applicant, $scope.student);

							console.log($scope.student);

					// Post To Server Via A Factory Service...........................


							ConfirmationService.AddConfirmationToDB($scope.student).then(function(response){
								
							     alert('confirmation was created successfully.');
							     $scope.success = true;
							    $scope.error = false;
							    $scope.successMessage = 'confirmation was created successfully.';
							    $scope.closeModalSave('#myModalSave');
							   $scope.getAllConfirmation();
							}, function(response){
								
							    $scope.errorMessage = "There was an error" + response.status;
							});

				}
			
			 }
//$scope.saveNewConfirmation();
...
$scope.printConfirmation = function(selectConfirm){
	...
	$http.get("/users/permitted_users").then(function(response){
		...
		if( ($scope.signatory[0][0]== null) || ($scope.signatory[1][0] == null)) {
			...
			$scope.$watch('selectConfirm', function(newValue, oldValue){
				...
				ResultService.ResultDetailsFromDB(newValue.exam_no, newValue.year_id, newValue.diet_id)
						.then(function(response){
...
angular.module('verifier').controller('DocPrinterController',["$scope", "$http", "$routeParams",'$window','ConfirmationService', '$location',
