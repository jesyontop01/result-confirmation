...
// Establish Connection To WES and Upload File  getAllApiResponses

$scope.getAllApiResponses = function(){
	$scope.loading = true;
		   $http({
		   method: 'GET',
		   url: '/web_services/api_transaction_responses.json',
		   }).then(function(response){
					   $scope.confirms = response.data;
					   console.log($scope.confirms);
...
var webClientPromise = {};
	   if ($routeParams.id < 3 ) {
		   webClientPromise = WebClient.getWebServicesById($routeParams.id);
	   } else {
		   webClientPromise = WebClient.getWebServices();
	   }

	   // WebClient.getWebServices().then(function(response){
	   webClientPromise.then(function(response){
		   $scope.loading = false;
   ...
   $scope.getAllAppServices = function () {
	   // body...
	   $scope.loading = true;

	   WebClient.getAppServiceTypes().then(function(response){
		   $scope.loading = false;
		   // $scope.webClients  = response.data.data;
		   // console.log($scope.webClients );
		   if (response.data.success == true) {
				   $scope.appServices  = response.data.data;
...
$scope.ServiceTypeSelection = function(id) {
	// body...
	$scope.loading = true;
//debugger
	...
	window.sessionStorage.setItem('App_Service_Type', $scope.webAppType.ServiceType);

	if ($scope.webAppType.ServiceType === "Send e-Mail") {

		$location.path("/result/mailServices/" + $scope.webAppType.id + "/" + $routeParams.id );

	} else {
		$location.path("/result/webServices/" + $scope.webAppType.id + "/" + $routeParams.id );
	}
...
$scope.ConnecttoMailService = function(id) {
	// body...
	$scope.loading = true;
//debugger
	...
	$location.path('/result/eMail_upload/'+ $routeParams.id + "/"+ $routeParams.confirmId);
...
//debugger
	...
	let usernameWES = $scope.clientSelected.username;//  "WAECNGA";
	let passwordWES =  $scope.clientSelected.password;// "2rDYxvSh6VW2";
	let clientID =   $scope.clientSelected.id;
...
$scope.getConfirmByconfirmId = function(confirmId) {
	// body...
	$scope.loading = true;
	confirmId = $routeParams.confirmId
...
$scope.getConfirmByID = function(confirmId) {
	...
	$scope.confirm = {};
	debugger
	$http.get('/confirmations/'+ $routeParams.id +'.json')
		  .then(function(response) {
		  	// body...
		  $scope.loading = false;
		  //debugger
	...
	debugger
			...
			if (confirm.user_id != user.id) {
				...
				SweetAlert.swal("Warning!", "Sorry "+ user.title +" "+ user.surname+", the officer that created this confirmation must be logged on before it may be printed!");
...
var isBlankSignature = false;
			...
			if (userSession.getCookieData() != null) {
  	 		...
  	 		$scope.userID = $scope.usrSecond.userID;
				 ...
				 $http.get("/signatures/users_signatures",{"params": { "email": $scope.userID}} ).then(function(response){
					...
					//console.log(response.data);
					debugger
					$scope.ServiceType = window.sessionStorage.getItem('App_Service_Type');
  ...
  //Generating Confirmation of Results Without Emborsed Signature
		...
		$scope.generateConfirmation2 = function(confirm) {
			...
			debugger
			var isBlankSignature = false;
			$scope.printConfirmation10($scope.selectConfirm, isBlankSignature);
						...
						// 		$scope.printConfirmation11($scope.selectConfirm, isBlankSignature);
						// 		//$route.reload(); 
...
//console.log( $scope.confirmType);
 ...
 $scope.hisHers = function(sex){
  ...
  $scope.heShes = function(sex){
	...
	$scope.formRecipientAddress = function (newValue) {
		var body = []
		...
		if (window.sessionStorage.getItem('App_Service_Type') === "Send Via API") {
			...
			if (newValue.WES_Ref === null || newValue.WES_Ref === "") {
				body = 		[
					...
					//{text: 'WES REFERENCE NO.: '+newValue.WES_Ref}
				  ]
			} else {
				body = 		[
					{text: newValue.dest_title},
	...
	$scope.formPDFAddressBarTable = function (resultDetail, centre, candNo, newValue){

		var body = [];

		if (centre.startsWith('4')) {
			...
			body = 	[ 

				{
					...
					table: {
						...
						widths: [150, '*'],
						body: [
							['NAME OF EXAM :', {text: resultDetail[0].examType}],
							['NAME OF SCHOOL :', {text: resultDetail[0].CentreName.trim()}],
							[ 'CANDIDATE\'S NUMBER :', {text: centre+' / '+candNo }],
							[ 'CANDIDATE\'S NAME :', {text: resultDetail[0].candName}],
							[ 'ADDRESS :', {text: newValue.Cand_address}],
							[ 'SEX :', {text: resultDetail[0].sex}],
							[ 'DATE OF BIRTH :', {text: resultDetail[0].dob2}],
						]
					}
				},
	...
	$scope.formPDFSignatoryBar = function (signatory, isBlankSignature){

		var body = [];

		if (isBlankSignature == true) {
					...
					stack:[
							{
								 ...
								 image: 'data:image/jpg;base64,'+signatory[0][1],
								  width: 200
								//image: 'image/waec.jpeg'
							},
						{text: signatory[0][0][1].toUpperCase() + " "+ signatory[0][0][2].toUpperCase()+ " " + "(" + signatory[0][0][0].toUpperCase() +")" , bold:true},
					...
					stack:[
							{
								 ...
								 image: 'data:image/jpg;base64,'+signatory[1][1],
								  width: 200
								//image: 'image/waec.jpeg'
							},
						{text: signatory[1][0][1].toUpperCase() + " "+ signatory[1][0][2].toUpperCase()+ " " + "(" + signatory[1][0][0].toUpperCase() +")" , bold:true},
...
//console.log(resultDetail);
		  ...
		  for (var key in rows){
		    ...
		    if (rows.hasOwnProperty(key))
		    {
			  ...
			  row.push(data.GradeInter.toString());
		    //   row.push($scope.gradeIntepretation( data.Grade.toString()) );
		       //row.push($filter('date')(data.created_at, 'dd-MM-yyyy')); GradeInter
		      body.push(row);
...
//		console.log(body);
	...
	$scope.printConfirmation3 = function(selectConfirm, wesApplicantID, isBlankSignature){
...
debugger
				SweetAlert.swal({
					title: "Second signatory Log In?",
					text: "Sorry!, You Can Not Print Right Now, You Need A Second Signatory Or An AR To Effect Printing. \n Click OK to continue or NO to abort",
					type: "warning",
					showCancelButton: true,
					confirmButtonColor: "#DD6B55",confirmButtonText: "OK, Log In!",
		...
		if (newValue.IsVeriferResult == false) {
			...
			promisedObject.then(function(response){
			...
			promisedObject = ResultService.ResultDetailsFromVerifierDB(newValue.exam_no, newValue.diet_id, newValue.id);

			promisedObject.then(function(response){
				$scope.resultDetail = response.data.results[1];
				console.log($scope.resultDetail);
...
debugger
								...
								content: [
									...
									},
									{
									  ...
									  columns:[
												...
												//   {text: 'WES REFERENCE NO.: '+newValue.WES_Ref}
												// ]
												$scope.formRecipientAddress(newValue)
												,
						...
						
									{
									   margin:[0,4,0,5],
										...
										fontSize: 11,
										bold:true,
									  columns: $scope.formPDFAddressBarTable($scope.resultDetail, centre, candNo, newValue)
									},
								  ...
								  $scope.formPDFSignatoryBar($scope.signatory, isBlankSignature),
								//   {
								// 	margin:[0,2,0,5],
								// 	stack:[
								// 			{
														  ...
														  columns : [
																						{
																						   alignment : 'left',
																							...
																							stack: [
																								...
																								{
																								  text : 'DR. AMOS J. DANGUT, FSTAN, MNIM' ,
	...
	$scope.selectConfirm = selectConfirm;  
		...
		if (newValue.IsVeriferResult == false) {
			promisedObject = ResultService.ResultDetailsFromDB(newValue.exam_no, newValue.examYear, newValue.DietName);
			...
			promisedObject = ResultService.ResultDetailsFromVerifierDB(newValue.exam_no, newValue.diet_id);
...
$scope.printConfirmation10 = function(selectConfirm, isBlankSignature){
//console.log(selectConfirm);
...
$scope.selectConfirm = selectConfirm;  
  ...
  //         }
	...
	if (newValue.IsVeriferResult == false) {
		debugger
		promisedObject = ResultService.ResultDetailsFromDBForConfirmation(newValue.exam_no, newValue.examYear, newValue.DietName, newValue.id, newValue.isPrinted);
...
$scope.printConfirmation11 = function(selectConfirm, isBlankSignature){
//console.log(selectConfirm);
